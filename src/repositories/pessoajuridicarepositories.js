...
const findByEmailPessoaJuridicaRepository = (email) =>
  PessoaJuridica.findOne({ email: email }).select("+password");

const createServiceRepository = (body) => PessoaJuridica.create(body);

const findAllServiceRepository = () => PessoaJuridica.find();

const findByIdServiceRepository = (idPessoaJuridica) =>
  PessoaJuridica.findById(idPessoaJuridica);

const updateServiceRepository = async (
